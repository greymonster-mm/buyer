	...
	<Col span="16" offset="4" >
	    <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="120">
	        ...
	        <FormItem required label="产品分类" prop="category" >
	        	<Tree :data=categorys accordion=true default-expand-all=false ref="tree" id="cat_tree_edit" :multiple=tree_multi  @on-select-change="select_category"></Tree>
	        </FormItem>
	        ...
	        <FormItem label="自己售价" prop="other_price">
	       		<Input type="number" v-model="formValidate.other_price"  placeholder="请输入自己对该商品的售价"></Input>
	        </FormItem>
	        <FormItem label="参考价" prop="suggested_price">
	        	<Input type="number" v-model="formValidate.suggested_price"  placeholder="请输入淘宝／京东 等其他平台售价（参考）"></Input>
	        </FormItem>
  ...
  methods: {
    ...
    handleSubmit (name) {
    	...
    	for(i = 0; i < check_img.length; i++)
    	{
    		...
    		{
    			this.$Message.error("图片尚未上传完毕!请上传完毕后再添加!");
    			return false;
    		}
    	}
        this.$refs[name].validate((valid) => {
            if (valid) {
	            var that = this
		     	...
		     	that._httpRequest({'url': 'updateGood', 'params' : params}).then(function (result)
		  	    {
		          if (!result.code)
		          {
	...
	{
  	  var that = this
  	  that._httpRequest({'url': 'getCategoryTree', 'params' :{'pid': pid, 'expand': true}}).then(function (result)
  	  {
        if (!result.code)
        {
          return
    ...
    getGood :function(params){
	      var that = this;
	      if (that.id == 0) return
	      var params = params ? params : {'id': that.id}
	      that._httpRequest({'url': 'getGood', 'params': params}).then(function (result) {
	        ...
	        if (that.good)
	        {
	        	var nodes = document.getElementById("cat_tree_edit").getElementsByClassName('ivu-tree-title')
		     	var i = 0
		     	for(i; i<nodes.length; i++)
		     	{
		     		if(nodes[i].textContent == that.good.cname)
		     		{
			     		var click = document.createEvent('MouseEvent');
						click.initEvent('click', false, false);
		     			nodes[i].dispatchEvent(click)
		     		}
		     	}
	        	that.select_category()
	        	that.formValidate['brand'] = that.good.brand
		        that.formValidate['product_name'] = that.good.product_name
		        that.formValidate['category'] = that.good.category
		        that.formValidate['description'] = that.good.description
		        that.formValidate['ad_copy'] = that.good.ad_copy
		        that.formValidate['price'] = that.good.price
		        that.formValidate['suggested_price'] = that.good.suggested_price
		        that.formValidate['other_price'] = that.good.other_price
		        var if_valid;
		        if(that.good.if_valid)
		        {
		        	if_valid = true;
		        }else
		        {
		        ...
		        that.formValidate['if_valid'] = if_valid
		        i = 0;
		        var tmpList = []
		        var tmp = {}
		        for(i; i < that.good.images.length; i++)
		        {
		        	tmp = {
		        		name: "pic",
		        		url: that.good.images[i].img_path,
		        		showProgress: false,
  						status: "finished"
		        	}
		        	that.$refs.upload.fileList.push(tmp)
